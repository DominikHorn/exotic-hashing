 ...
 */
   ...
   std::vector<Data> load_cached(ID id, size_t dataset_size) {
      ...
      static std::vector<Data> ds_amzn, ds_fb, ds_osm, ds_wiki;
      ...
      switch (id) {
         ...
         case ID::NORMAL: {
            ...
            for (size_t i = 0; i < ds.size(); i++) {
               ...
               const auto rand_val = std::max(mean - 3 * std_dev, std::min(mean + 3 * std_dev, dist(rng)));
         ...
         case ID::BOOKS: {
            if (ds_amzn.empty()) {
               ds_amzn = load<Data>("data/books_200M_uint64");
               std::shuffle(ds_amzn.begin(), ds_amzn.end(), rng);
            }
            ...
            for (size_t i = 0; i < ds_amzn.size() && i < ds.size(); i++)
               ds[i] = ds_amzn[i];
            break;
         }
         case ID::FB: {
